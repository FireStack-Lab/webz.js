...
var encodeTransaction = exports.encodeTransaction = function encodeTransaction(txn) {
  ...
  var encoded = (0, _transformer.intToByteArray)(txn.version, 64).join('') + (0, _transformer.intToByteArray)(txn.nonce, 64).join('') + txn.to + txn.pubKey
  // + txn.amount.toString('hex', 64)
  // update later
  + (0, _transformer.intToByteArray)(txn.amount, 64).join('') + (0, _transformer.intToByteArray)(txn.gasPrice, 64).join('') + (0, _transformer.intToByteArray)(txn.gasLimit, 64).join('') + (0, _transformer.intToByteArray)(txn.code.length, 8).join('') // size of code
