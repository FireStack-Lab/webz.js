...
var verifyPrivateKey = function verifyPrivateKey(privateKey) {
  ...
  return _bcrypto.secp256k1.privateKeyVerify(privateKey);
};
...
var getAddressFromPrivateKey = function getAddressFromPrivateKey(privateKey) {
  ...
  var pubKey = _bcrypto.secp256k1.publicKeyCreate(privateKey, true);
...
var getPubKeyFromPrivateKey = function getPubKeyFromPrivateKey(privateKey) {
  ...
  return _bcrypto.secp256k1.publicKeyCreate(privateKey, true);
};
...
var createTransactionJson = function createTransactionJson(privateKey, txnDetails) {
  ...
  var codeHex = new Buffer(txn.code).toString('hex');
  var dataHex = new Buffer(txn.data).toString('hex');
  ...
  var messageHex = new Buffer(msg, 'hex');

  var sig = schnorr.sign(messageHex, privateKey, pubKey);
